...
//
//This block tests whether or not the scheduler can correctly identify that two registers contain values that are different offsets from the same memory value, and therefore must be different, so their stores and loads $
